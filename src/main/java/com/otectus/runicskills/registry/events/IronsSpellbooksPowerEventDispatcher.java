 ...
 *
 * <p><b>Which Powers this file implements is not restated here.</b> It used to be, as a
 * hand-maintained bullet list, and it drifted: it named Marrow Sense as wired end-to-end long
 * after that Power was declared {@code INERT}, so the one place a reader would look for coverage
 * disagreed with both machine-checked sources. The authoritative answers are
 * {@link com.otectus.runicskills.registry.content.ContentStatusIndex} at runtime and
 * {@code src/test/resources/power_no_effect_allowlist.txt} at build time, and
 * {@code ContentStatusTest} fails the build if those two ever disagree with each other.
 *
 * <p>Where a Power ships something narrower than its design describes, the substitution and the
 * reason for it are documented at that Power's own dispatcher case, next to the code, rather than
 * in a summary at the top of the file.
 */
public class IronsSpellbooksPowerEventDispatcher {
    ...
    private static void detonatePyroclasm(Player owner, LivingEntity corpse, Power pyro) {
        ...
        // around them would put the ring in the one place the explosion did not happen.
        com.otectus.runicskills.registry.powers.PowerDispatch.fireProc(owner, pyro, corpse,
                corpse.position(), 0, 200, false, false);
    }
     ...
     *
     * <p>Occupying a slot is necessary but not sufficient. This used to check registry presence,
     * disabled state and the stored slot only — so an operator who lowered a player's skill or
     * raised a requirement left the effects firing indefinitely, and none of the Seal, Crown or
     * prerequisite rules were consulted at proc time at all (RS10-006). Equipping and proccing now
     * run the same evaluation, minus the slot-capacity and budget checks that only apply when
     * taking a slot.
     */
    public static boolean isEquipped(Player player,
        ...
        Power p = ro.get();
        SkillCapability cap = SkillCapability.get(player);
        if (cap == null || !cap.isPowerEquipped(p)) return false;
        return com.otectus.runicskills.registry.powers.PowerEligibility.evaluateActive(player, p).eligible();
    }

    /**
     * Delegates to the shared helper, which fires {@link com.otectus.runicskills.event.PowerProcEvent}
     * and sends the presentation. This used to build and send the packet itself, so the ISS half's
     * procs bypassed the throttle and the event that the vanilla half went through.
     */
    ...
    /** As {@link #fireProc(Player, Power)}, naming the entity the proc acted on. */
    private static void fireProc(Player player, Power p, net.minecraft.world.entity.Entity target) {
