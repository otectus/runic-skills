...
public class HandlerCommonConfig {
    ...
    @Clamp(min = 1.0f, max = 8.0f)
    public float cleaveRangeBlocks = 3.0f;

    @SerialEntry(comment = "Cleave perk: maximum number of extra enemies a single swing can splash onto. Bounds the tick cost of swinging into a large crowd.")
    @AutoGen(category = "common", group = "perks")
    ...
    @SerialEntry(comment = "Locksmith: Minutes before the same container can grant experience again. Prevents reopening one chest as an unlimited XP source. 0 disables the per-container cooldown.")
    @AutoGen(category = "common", group = "tinkering")
    @IntField(min = 0, max = 1440)
    @Clamp(min = 0, max = 1440)
    public int locksmithContainerCooldownMinutes = 20;

    @SerialEntry(comment = "Locksmith: Minimum seconds between any two Locksmith experience rewards, regardless of which container was opened. Bounds the reward rate for players with many chests.")
    @AutoGen(category = "common", group = "tinkering")
    @IntField(min = 0, max = 600)
    @Clamp(min = 0, max = 600)
    public int locksmithMinSecondsBetweenRewards = 5;
