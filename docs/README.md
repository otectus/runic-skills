...
| Path | Status |
| --- | --- |
| `src/main/resources/assets/runicskills/textures/skill/**` | **Shipped.** Skill rank, Perk and Passive icons. Generated by [`../tools/icongen/build.py`](../tools/icongen/build.py) from the authored masks and `specs*.tsv`; see the [art guide](../tools/icongen/README.md). Existing GUI frame textures remain hand-authored |
| `src/main/resources/assets/runicskills/textures/power/**` | **Shipped, generated.** Explicit mechanic silhouettes and badges from `specs_powers.tsv`, with tiers/schools read from `RegistryPowers`. `RunicIconIntegrityTest` and `PowerIconCoverageTest` check decoded pixels, native size, alpha, palette, registration coverage and uniqueness |
