> **Historical document — Deferred items — post-1.3.7 (June 2026)**
>
> Items deliberately not changed during the 1.3.7 configuration audit, kept for provenance. **Several are now resolved and are not marked as such here**: `/registeritem` no longer throws on the default lock list, and the `BotaniaIntegration` TODO refers to an integration removed in 1.5.0. The protocol number it quotes (5) is four bumps out of date — it is 10.
>
> Open work is tracked in [`docs/RUNIC_SKILLS_1_10_AUDIT_AND_UPDATE_PLAN.md`](../RUNIC_SKILLS_1_10_AUDIT_AND_UPDATE_PLAN.md).
