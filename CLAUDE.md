...
src/main/java/com/otectus/runicskills/
  ├── mixin/           — Mixins (30 total: 5 client, 25 common)
...
- **MixinExtras** (0.4.0) — declared as an annotation processor and a jar-in-jar dependency, but
  **not used by any mixin in this project**, and `jarJar` is never enabled. Kept only because
  removing it is a build change nobody has needed yet; do not reach for it expecting it to be wired.
...
- Mixins: declared in `runicskills.mixins.json`. Plain SpongePowered Mixin — no MixinExtras
  annotations are used anywhere, despite the dependency being declared (see above)
...
Common (25): AbstractContainerMenu, AbstractFurnaceBlockEntity, AbstractMagicProjectile,
AbstractMinecart, BrewingStandBlockEntity, CraftingMenu, CreeperHeadProjectile, DispenserBlock,
EnchantmentMenu, GrindstoneMenu, HopperBlockEntity, ItemStack, LivingEntity, LivingEntityAccess,
Player, PlayerAction, PowderSnowBlock, ReforgingResultSlot, SalvagingMenu, ShulkerBullet, Slot,
StonecutterMenu, ThrownPotion, TrueInvisibilityEffect, Villager
