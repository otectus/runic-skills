...
**Inventory tabs now stay out of the way.** They avoid the recipe book, potion effects, other mods' tab strips and side buttons, and any custom regions. You can move the tab strip with Shift-drag, and the position is saved. With CustomNPCs, the Skills tab joins CustomNPCs' own tab strip instead of adding a second one.
